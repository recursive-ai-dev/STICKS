// make_apngs.js
// Create APNGs per character+direction from frames named like:
...
const INPUT_DIR = path.resolve(getArg("in", "./medieval_sprites"));
const OUTPUT_DIR = path.resolve(getArg("out", "./medieval_apng"));
...
// Match: 12_right_3.png
const FRAME_RE =
  /^(\d+)_(front|right|back|left)_(\d+)\.(?:png|PNG|apng|APNG)$/;

function groupFrames(dir) {
  const files = fs.readdirSync(dir).filter((f) => FRAME_RE.test(f));
  const map = new Map(); // key: `${id}_${direction}` -> [{idx, file}]
  for (const f of files) {
    ...
    map.get(key).push({ idx, file: path.join(dir, f) });
  }
  // Sort frames
...
(async () => {
  ...
  if (groups.size === 0) {
    console.log("No matching frames found. Expected files like 1_right_0.png");
    process.exit(0);
  }
  ...
  console.log(
    `\nDone. Created ${made} APNG(s) from ${groups.size} sequence(s) at ${FPS} fps.`
  );
})();